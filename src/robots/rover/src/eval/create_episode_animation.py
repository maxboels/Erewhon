...
def create_episode_animation(episode_dir: Path, output_dir: Path, fps: int = 10, output_format: str = 'mp4', skip_existing: bool = True):
    """Create animated video for a single episode"""
    
    # Check if animation already exists
    output_file = output_dir / f"{episode_dir.name}_animation.{output_format}"
    if skip_existing and output_file.exists():
        print(f"⏭️  Skipping {episode_dir.name} - animation already exists: {output_file.name}")
...
def create_combined_animation(data_dir: Path, output_dir: Path, fps: int = 8, output_format: str = 'mp4', skip_existing: bool = True):
    """Create a combined animation showing multiple episodes"""
    
    # Check if combined animation already exists
    output_file = output_dir / f"combined_episodes_animation.{output_format}"
    if skip_existing and output_file.exists():
        print(f"⏭️  Skipping combined animation - already exists: {output_file.name}")
        return str(output_file)
...
def main():
    ...
    parser.add_argument('--data_dir', type=str,
                        default='../../episodes',
                        help='Path to data directory (default: ../../episodes relative to this script)')
    ...
    parser.add_argument('--force', action='store_true',
                        help='Force recreation of animations even if they already exist')
    ...
    for episode_dir in episode_dirs:
        result = create_episode_animation(episode_dir, output_dir, args.fps, args.format, skip_existing)
        if result:
    ...
    if args.combined and len(episode_dirs) > 1:
        print("\n" + "=" * 60)
        combined_result = create_combined_animation(data_dir, output_dir, max(6, args.fps//2), args.format, skip_existing)
        if combined_result:
